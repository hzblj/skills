...
scripts/         # gen-readmes.mjs, gen-openai-yaml.mjs, link-skills.sh, list-skills.sh
...
- **New skill:** create `skills/<platform>/<category>/<name>/SKILL.md` with
  ...
  — otherwise it won't ship via the plugin or skills.sh (see Distribution above). Then
  run `yarn openai-yaml` to generate its `agents/openai.yaml` (see Codex config below).
...
- After adding, removing, or renaming a skill, run `yarn readmes` to regenerate the
  ...
  It walks `skills/` only, so it never touches the root `agents/`/`commands/`, and it
  stops at a skill's own folder so it never writes into a skill's `agents/` subfolder.
- **Codex config:** every leaf skill carries an `agents/openai.yaml` — the OpenAI
  Codex / ChatGPT skill config (`interface.display_name`, `short_description`,
  `policy.allow_implicit_invocation`). It's derived from the SKILL.md frontmatter and
  regenerated by `yarn openai-yaml` ([scripts/gen-openai-yaml.mjs](./scripts/gen-openai-yaml.mjs)),
  so **never hand-edit it** — change the SKILL.md and regenerate. Claude Code and
  skills.sh→Claude ignore it; it only affects OpenAI harnesses. Note this per-skill
